...
@session_management_bp.route('/terminate_all_other_unnamed_sessions', methods=['POST'])
@requires_auth
def terminate_all_other_unnamed_sessions():
    session_token = request.cookies.get('session_token')
    if not session_token:
        return make_toast(400, _("No session token found"))
    ...
    # Delete all unnamed sessions for this user except the current one
    deleted_count = db.session.query(Session).filter(
        Session.user_uuid == current_session.user_uuid,
        Session.uuid != session_token,
        Session.name == ""
    ...
    return make_toast(200, _("Terminated {} unnamed session(s)").format(deleted_count))
